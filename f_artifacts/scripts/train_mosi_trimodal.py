...
def compute_metrics(y_raw, pred_cls):
    ...
    mask = y_raw != 0
    if mask.sum() > 0:
        acc_2_no_neutral = accuracy_score((y_raw[mask] > 0).astype(int), (pred_score[mask] > 0).astype(int))
        f1_2_no_neutral = f1_score((y_raw[mask] > 0).astype(int), (pred_score[mask] > 0).astype(int), average="weighted", zero_division=0)
    else:
...
def train_one(name, model, cfg, train_loader, val_loader, test_loader, device, aspect_loss_weight=0.0):
...
def main():
    ...
    train_ds = MOSIPKLDataset(cfg["data_path"], "train", max_context=cfg["max_context"], num_aspects=cfg["num_aspects"], use_vision=cfg["use_vision"])
    val_ds = MOSIPKLDataset(cfg["data_path"], "valid", max_context=cfg["max_context"], num_aspects=cfg["num_aspects"], use_vision=cfg["use_vision"])
    test_ds = MOSIPKLDataset(cfg["data_path"], "test", max_context=cfg["max_context"], num_aspects=cfg["num_aspects"], use_vision=cfg["use_vision"])
    ...
    rows.append(train_one("DC2M_Trimodal", baseline, cfg, train_loader, val_loader, test_loader, device))
    ...
    rows.append(train_one("GRAPH_SA_Trimodal", graphsa, cfg, train_loader, val_loader, test_loader, device, aspect_loss_weight=cfg["aspect_loss_weight"]))

    df = pd.DataFrame(rows)
    os.makedirs("outputs", exist_ok=True)
    df.to_csv("outputs/mosi_trimodal_results.csv", index=False)

    print("\n========== MOSI Trimodal Results ==========")
